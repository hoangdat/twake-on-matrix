...
class ChatListItemSubtitle extends StatelessWidget with ChatListItemMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return FutureBuilder<Event?>(
      future: _getLastEvent(context),
      builder: (context, snapshot) {
        final lastEvent = snapshot.data;
        print('datph at room ${room.getLocalizedDisplayname(MatrixLocals(L10n.of(context)!))} chatListItemSubtitle at room: lastEvent = ${jsonEncode(lastEvent)}');
          ...
          children: <Widget>[
            Expanded(
              child: _buildMainSubtitleContent(
                  context, lastEvent, typingText, isGroup, isMediaEvent,),
            ),
  ...
  Future<Event?> _getLastEvent(BuildContext context) async {
    return room.lastEventAvailableInPreview(context);
  }
